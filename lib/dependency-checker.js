...
EmberCLIDependencyChecker.prototype.readShrinkwrapDeps = function() {
  ...
  if (fileExists(filePath)) {
    ...
    return ShrinkWrapChecker.checkDependencies(this.project.root, this.project.nodeModulesPath, shrinkWrapJSON);
  } else {
    return [];
  }
};
...
EmberCLIDependencyChecker.prototype.lookupNodeModuleVersion = function(name) {
  var nodePackage = path.join(this.project.nodeModulesPath, name, 'package.json');
